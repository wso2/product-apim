...
public class AdditionalPropertiesInAPIListingTestCase extends APIMIntegrationBaseTest {
    ...
    // The tests share the single API created above and are chained with dependsOnMethods, so that one broken
    // expectation is reported once instead of repeating as a failure in every test that follows it.
