...
public class AsyncAPITestWithValidationCase extends APIMIntegrationBaseTest {
    ...
    private final String INVALID_ASYNC_API_V3_NAME = "InvalidAsyncAPIV3";
    private final String INVALID_ASYNC_API_V3_CONTEXT = "invalid/async/v3";
    private final String INVALID_ASYNC_API_V3_VERSION = "1.0.0";
    ...
    @Test(description = "Import an Invalid Async API V3")
    public void testPublishInvalidAsyncApiV3() throws Exception {
        asyncAPIDefinition = IOUtils.toString(
                getClass().getClassLoader().getResourceAsStream("async" + File.separator + "asyncapiv3_invalid.yaml"),
                StandardCharsets.UTF_8);

        File file = getTempFileWithContent(asyncAPIDefinition);

        JSONObject additionalPropertiesObj = new JSONObject();
        additionalPropertiesObj.put("name", INVALID_ASYNC_API_V3_NAME);
        additionalPropertiesObj.put("context", INVALID_ASYNC_API_V3_CONTEXT);
        additionalPropertiesObj.put("version", INVALID_ASYNC_API_V3_VERSION);
        additionalPropertiesObj.put("type", ASYNC_TYPE);
        ...
        // create Async API V3 with invalid def — expect the parser to reject it
        ApiResponse<APIDTO> response = null;
        try {
            response = restAPIPublisher
                    .importAsyncAPIDefinition(file, additionalPropertiesObj.toString());
            ...
            String respBody = e.getResponseBody();
            Assert.assertTrue(respBody != null && respBody.contains("Operation channel reference must point " +
                    "to valid channel."),
                    "Unexpected AsyncAPI V3 validation response; body: " + respBody);
        }
        if (response != null) {
            Assert.fail("Invalid AsyncAPI V3 definition was accepted but should have been rejected");
        }
    }
