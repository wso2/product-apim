...
public static GovernableStatesEnum fromValue(String value) {
    ...
    **/
    ...
    /**
        * Comma separated rule severities that make this policy fail. Violations of other severities are still reported but do not affect the policy adherence status.  Three states are meaningful. Null means per policy severity filtering is not enabled on this deployment, and clients should not offer it. An empty string means it is enabled but nothing has been configured for this policy, so every severity affects compliance. A value lists the severities that do.  Allowed severities are &#x60;ERROR&#x60;, &#x60;WARN&#x60; and &#x60;INFO&#x60;, matched case-insensitively. Any other value is rejected with 400 and error code 990212. An accepted value is stored normalised: surrounding whitespace is dropped, case is folded to upper case and repeats are removed, so it may read back in a different form from the one sent. A selection sent to a deployment which has not enabled the feature is also rejected with 400, under error code 990213, rather than being accepted and dropped.  Omitting the field on an update preserves whatever is stored; sending it empty clears the selection. 
