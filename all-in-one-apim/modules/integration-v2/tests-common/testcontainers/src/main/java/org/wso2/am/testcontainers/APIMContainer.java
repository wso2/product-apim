...
//        withLogConsumer(new Slf4jLogConsumer(logger).withPrefix(containerLabel));
//
//        Slf4jLogConsumer logConsumer = new Slf4jLogConsumer(logger).withMdc("key", "value");
        ...
        // send container logs with MDC fields so Logback can route them
        Slf4jLogConsumer logConsumer = new Slf4jLogConsumer(logger)
                .withPrefix(containerLabel)
                .withSeparateOutputStreams()     // stdout=INFO, stderr=ERROR
                .withMdc("testName", testName);   // per-<test> routing
