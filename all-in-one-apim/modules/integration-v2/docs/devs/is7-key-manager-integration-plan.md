...
`@type`/`@dep:gateway` per scenario; features live in `features/admin/`. 54 scenarios render in the
...
## Parked / follow-ups
- **Multi-tenant SSO lane** (`MultiTenantSsoRunner` in the dedicated
  `IntegrationV2-MultiTenantSso` block of `testng-v2.xml`, and
  `features/admin/multitenant_sso.feature` with the
  `artifacts/configFiles/multiTenantSso/` overlays): kept as its own block
  because it requires the tenant-synchronization listener and distinct broker
  configuration. It boots a second IS with a tenant-sync overlay and uses the
  per-block network model, so its `wso2is` and `wso2am` aliases remain isolated.
