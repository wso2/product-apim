...
(`../../docs/devs/coverage-tree.md` — GENERATED by `render_coverage_tree.py` from `capability-map.yml` +
the actual `.feature` tags, so it cannot go stale; regenerate rather than hand-edit) and the existing
`.feature` files for the capability you're about to test. The tree tells you *where a test belongs*; only
the feature files tell you *what is already covered* — never judge coverage from the tree or from tag
names alone. Extend an existing feature/scenario if one fits; only add a new file when nothing does.
