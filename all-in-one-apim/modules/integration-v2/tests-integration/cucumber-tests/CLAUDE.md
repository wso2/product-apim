...
- `getBaseUrl()` / `getBaseGatewayUrl()` / `getBaseGatewayWsUrl()` / `getBaseGatewayWssUrl()` /
  `getBaseWebSubEventReceiverUrl()` — the block's published base URLs from shared context, throwing a clear
  "not booted yet" when absent. Never re-read `TestContext.get("baseUrl")` by hand, and never rebuild one of these
  from a literal port (the WebSub event receiver is a SEPARATE listener from the gateway passthrough, so it has its
  own accessor rather than being derived from `getBaseGatewayUrl()`).
...
- **Three retry contracts, one shared loop — picking one IS the statement of intent** (like
  ...
  - `awaitSettledCount(quietMillis, timeoutMillis, probe)` → `SettledCount(value, settled, samples)` — the
    envelope when the assertion target is a MONOTONIC COUNTER's FINAL value ("exactly N events were delivered").
    Polls until the value has been unchanged for `quietMillis`, then the caller asserts `settled()` AND the exact
    value. Use it instead of `retryUntil` whenever an OVER-count must be catchable: `retryUntil`'s accept can only
    say "has it reached N", so accept-on-`>= N`-then-assert-`== N` passes the moment the counter touches N and an
    extra arrival moments later is invisible — the §12-forbidden widened form wearing an exact assertion. Sound
    ONLY for a counter that goes quiescent; pick `quietMillis` above the gap BETWEEN arrivals (not the total
    fan-out), since a longer gap settles early and UNDER-counts. First consumer: WebSub delivery counts (§11).
  - Using the gate for an assertion target swallows fail-fast errors and robs the step of its exact-value
    assertion; using `retryUntil` for prerequisite state never self-heals a dropped event; using `retryUntil` for
    a final count silently permits an over-count.
...
**`utils/HmacTestUtils`** (raw keyed-MAC digests + the WebSub signature convention — deliberately NOT folded into
`JwtTestUtils`, whose charter is JWT segment/assembly work):
- `hmacHex(macAlgorithm, data, key)` — RFC 2104 HMAC as lower-case hex (was the legacy
  `StreamingApiTestUtils.calculateRFC2104HMAC`).
- `hubSignature(signingAlgorithm, body, secret)` — the `x-hub-signature` value WebSub prescribes (`sha1=<hex>`),
