...
Feature: MCP Server authoring (publisher plane)
  ...
  #
  # The IMPORT alone is performed with the actor's ADMIN token: it accepts apim:mcp_server_import_export — which
  ...
  # why the actor column is the tenant ADMINS rather than the publisherUser pair its siblings run over.
  @cap:publisher @feat:mcp-servers @rule:cross-environment-import @type:regression
  Scenario Outline: An MCP server whose archive records a reference API id from another environment is imported as <actor>
