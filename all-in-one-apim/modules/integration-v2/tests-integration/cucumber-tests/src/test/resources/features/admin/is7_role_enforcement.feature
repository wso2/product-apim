...
Feature: External Key Manager Role-Based Authorization Enforcement
  ...
  Scenario: A user with the mapped IS7 role can invoke the scope-protected operation
    ...
    When I request an OAuth access token from the external key manager using password grant as "is7roleuser" with password "Wso2Test123!" requesting scope "{{is7ScopeName}}"
  ...
  Scenario: A user without the mapped IS7 role is forbidden from the scope-protected operation
    ...
    When I request an OAuth access token from the external key manager using password grant as "is7noroleuser" with password "Wso2Test123!" requesting scope "{{is7ScopeName}}"
    Then The response status code should be 200
