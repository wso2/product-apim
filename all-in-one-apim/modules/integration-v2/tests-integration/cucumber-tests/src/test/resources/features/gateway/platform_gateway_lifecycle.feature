...
Feature: API Platform Gateway lifecycle
  ...
  Scenario: Register a platform gateway, deploy an API-key-secured REST API, and enforce auth at the gateway
    ...
    # the key actually being served, re-deploying the API if the key never arrived; the invokes that follow still assert.
    Then the platform gateway serves the API key "pgApiKey" for API "pgApiId", re-deploying the API if the key never reaches the gateway
