Feature: Migrated API Revision Management
  ...
  Scenario Outline: Migrated API Revision Management
    ...
    When I get the existing revision as "existingRevisionID" for API with "<apiID>"
    ...
    When I deploy revision "revisionId" of API "<apiID>"
    Then The response status code should be 201

    # Step 5: Undeploy an existing API revision
    When I undeploy revision "revisionId" of API "<apiID>"
    Then The response status code should be 201

    # Step 6: Restore a previous revision to make it the current working copy
    When I restore a previous revision "revisionId" of API "<apiID>"
    Then The response status code should be 201

    # Step 7: Delete an unused or old revision, created revision
    When I Delete the API revision with "revisionId" for "<apiID>"
    Then The response status code should be 200
