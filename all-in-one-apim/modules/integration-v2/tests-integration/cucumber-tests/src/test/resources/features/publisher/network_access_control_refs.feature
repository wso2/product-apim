...
Feature: Publisher Network Access Control - Remote OpenAPI ref resolution
  ...
  "could not be resolved" error. Ports SafeRefResolutionTestCase (private-block group). Runs in the
  ...
  Scenario Outline: A loopback reference in a <variant> definition is rejected as <actor>
  ...
  Scenario Outline: A loopback reference in a <variant> OpenAPI archive is rejected as <actor>
    Given The system is ready and I have valid publisher access tokens as "<actor>"
