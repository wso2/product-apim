...
Feature: Publisher Network Access Control - Nested WSDL import
  ...
  and a "could not be resolved" error before any fetch. Ports HostValidationWsdlNestedImportTestCase. Runs in the
  ...
  Scenario Outline: A WSDL nested xsd:import via <target> is rejected as <actor>
