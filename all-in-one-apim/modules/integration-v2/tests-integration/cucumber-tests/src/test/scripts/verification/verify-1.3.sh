...
echo "VERIFY 1.3: DEPRECATED (superseded) - not run; see parallel-framework-implementation-plan.md"
exit 0
#
# Phase 1.3 verification — end-to-end API lifecycle through dynamic ports.
#
# Drives DCR -> token -> create -> deploy -> publish -> subscribe -> token -> invoke against a
# DynamicApimContainer on dynamic/ephemeral host ports and asserts the gateway invocation returns 200.
# Re-runnable / idempotent: pre-cleans any leftover verify-1.3 containers, runs the isolated Phase 1.3
# suite (which inherits the real surefire env), then asserts no Docker container leaks and that the
# recorded servlet-https host port is released. Prints a single PASS/FAIL line and exits non-zero on
# failure.
#
# Usage:  ./verify-1.3.sh
set -euo pipefail
...
PORT_FILE="${MODULE_DIR}/target/verify-1.3-servlet-https-port.txt"
...
echo "== Phase ${STEP} verification: end-to-end lifecycle through dynamic ports =="
rm -f "${PORT_FILE}"
cleanup_containers

echo "Running verification suite via Maven..."
if ! ( cd "${REACTOR_DIR}" && mvn -q -pl tests-integration/cucumber-tests -am \
        -Dsurefire.suite.xml=testng-fv-1.3.xml test ); then
...
# Post-exit assertion 2: the recorded mapped servlet-https host port is released (connection refused).
if [ ! -f "${PORT_FILE}" ]; then
...
echo "VERIFY ${STEP}: PASS - full lifecycle invoked through dynamic gateway port (200), port released, no leaks"
