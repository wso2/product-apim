...
public class Utils {
    ...
    /**
     * Replaces name placeholders of the form {@code ${UNIQUE:<base>}} and {@code ${LENGTH:<n>}} in a
     * JSON payload. {@code ${UNIQUE:<base>}} is substituted with a runner-unique value generated by
     * {@link Names#unique(String)}, letting payload files keep stable base names while every runner
     * instance gets distinct resource names/contexts. {@code ${LENGTH:<n>}} is substituted with a
     * runner-unique alphanumeric string of exactly {@code n} characters, generated by
     * {@link Names#ofLength(int)} — useful for boundary-length testing (e.g. a context field's max
     * allowed length) where the value must both hit an exact character count and stay unique across runs.
     *
    ...
    public static String resolvePayloadPlaceholders(String payload) {
        ...
        if (payload == null || (!payload.contains("${UNIQUE:") && !payload.contains("${LENGTH:"))) {
            return payload;
        }
        StringBuffer afterUnique = new StringBuffer();
        Matcher uniqueMatcher = UNIQUE_PLACEHOLDER.matcher(payload);
        while (uniqueMatcher.find()) {
            uniqueMatcher.appendReplacement(afterUnique, Names.unique(uniqueMatcher.group(1)));
        }
        uniqueMatcher.appendTail(afterUnique);

        StringBuffer result = new StringBuffer();
        Matcher lengthMatcher = LENGTH_PLACEHOLDER.matcher(afterUnique.toString());
        while (lengthMatcher.find()) {
            lengthMatcher.appendReplacement(result, Names.ofLength(Integer.parseInt(lengthMatcher.group(1))));
        }
        lengthMatcher.appendTail(result);
