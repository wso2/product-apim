...
public class APIStoreRestClient {
    ...
    public HttpResponse removeSubscription(String name,String version,String provider,int id) throws Exception {
        checkAuthentication();
        HttpResponse response = HttpRequestUtil.doPost(new URL(backEndUrl + "/store/site/blocks/subscription/subscription-remove/ajax/subscription-remove.jag")
                , "action=removeSubscription&name=" + name + "&version="+version+"&provider="+provider+"&applicationId="+id
                , requestHeaders);
        if (response.getResponseCode() == 200) {
            VerificationUtil.checkErrors(response);
            return response;
        } else {
            throw new Exception("Subscription Removal Failed " + response.getData());
        }
    }
