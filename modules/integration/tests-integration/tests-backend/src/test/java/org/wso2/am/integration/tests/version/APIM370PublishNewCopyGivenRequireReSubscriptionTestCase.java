 ...
 * Invoke Original API and New Version API from Store
 * Propagate Changes to API Gateway
 * Deprecate Old Versions
 * Require Re-Subscription
 */
public class APIM370PublishNewCopyGivenRequireReSubscriptionTestCase extends
...
//                new Object[]{TestUserMode.TENANT_ADMIN},
    ...
    public void testPublishNewCopyGivenRequireReSubscription() throws Exception {
        ...
        assertEquals(subscriptionResponse.getResponseCode(), Response.Status.OK.getStatusCode(),
               apiName + "API has not been subscribed");
        ...
        List<APIIdentifier> recentlyAddedAPIList = APIMTestCaseUtils.
                getAPIIdentifierListFromHttpResponse(apiStore.
                        getRecentlyAddedAPIs(superUser, apiLimit));
        Thread.sleep(4000);

        assertTrue(APIMTestCaseUtils.isAPIAvailable
                (apiIdentifierStoreNewVersion, recentlyAddedAPIList), "New Copy of the " +
                apiName + " is not available under Recently added API in store");
        assertFalse(APIMTestCaseUtils.isAPIAvailable
                        (apiIdentifierStoreOldVersion, recentlyAddedAPIList),"Original" + apiName +
        ...
        assertTrue(APIMTestCaseUtils.isAPIAvailable
                        (apiIdentifierStoreNewVersion, publishedAPIList),
                "New Copy of the " + apiName + " is not available in store");
        assertFalse(APIMTestCaseUtils.isAPIAvailable
                        (apiIdentifierStoreOldVersion, publishedAPIList),
...
//        Thread.sleep(8000);
        ...
        assertTrue(oldApiResponse.getData().contains(apiInvokeResponse),
                "Response data mismatched when api invocation for GET Request");

        Thread.sleep(4000);

        //Invoke New version of the API
        HttpResponse newApiResponse = HttpRequestUtil.doGet(gatewayUrlsWrk.getWebAppURLNhttp() +
                apiContext + "/" + apiVersionNew, requestHeaders);
        assertEquals(newApiResponse.getResponseCode(), Response.Status.FORBIDDEN.getStatusCode(),
                "Response code mismatched when invalid api invocation");

        //Re-subscribed the API
        ...
        //Check New Copy of the API under Subscribed APIs
        ...
        assertEquals(newVersionApiResponse.getResponseCode(), Response.Status.OK.getStatusCode(),
                "Response code mismatched when api invocation");
        assertTrue(newVersionApiResponse.getData().contains(apiInvokeResponse),
                "Response data mismatched when api invocation for GET Request");
    }
