...
public class AddEditRemoveRESTResourceTestCase extends APIManagerLifecycleBaseTest {
    ...
    private static final String API2_END_POINT_METHOD = "/";
    ...
    private String APPLICATION_NAME = "AddRemoveRESTResourceTestCase";
    ...
    public void testInvokePOSTAndGetResourceAfterAddingURLPattern() throws Exception {
        ...
        HttpResponse httpResponsePOSTInvalidUrl = HttpRequestUtil.doPost(new URL(API_BASE_URL + API_CONTEXT + "/" + API_VERSION_1_0_0 +
                API_POST_ENDPOINT_METHOD + INVALID_URL), "id=25", requestHeadersPost);
        assertEquals(httpResponsePOSTInvalidUrl.getResponseCode(),
                Response.Status.FORBIDDEN.getStatusCode(), "Invocation is not forbidden whentry to invoke POST resource  via invalid url pattern");
        assertTrue(httpResponsePOSTInvalidUrl.getData().contains(INVALID_URL_INVOKE_RESPONSE), "Invocation is not forbidden when" +
                " try to invoke POST resource  via invalid url pattern. Expected value :\"" + RESPONSE_GET + "\" not contains" +
                " in response data:\"" + httpResponsePOSTInvalidUrl.getData() + "\"");
    }
