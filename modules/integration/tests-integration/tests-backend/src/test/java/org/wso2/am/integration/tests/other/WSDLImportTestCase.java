...
@SetEnvironment(executionEnvironments = { ExecutionEnvironment.ALL }) public class WSDLImportTestCase
    ...
    private String WSDL_ZIP_IMPORT_API_NAME = "WSDLZipImportAPI";
    private String WSDL_ZIP_IMPORT_API_CONTEXT = "wsdlzipimport";
    ...
    public void testWsdlDefinitionImport()throws Exception {
        ...
        //create API by importing the WSDL definition as .zip file
        String wsdlZipPath = FrameworkPathUtil.getSystemResourceLocation()  + "wsdl" + File.separator + "Sample.zip";
        File file2 = new File(wsdlZipPath);
        APIDTO apidtoZip = restAPIPublisher
                .importWSDLSchemaDefinition(file2, backendEndUrl, additionalPropertiesObj.toString(), "SOAP");

        //Make sure API is created properly
        assertEquals(apidtoZip.getName(), WSDL_ZIP_IMPORT_API_NAME);
        assertEquals(apidtoZip.getContext(), "/" + WSDL_ZIP_IMPORT_API_CONTEXT);
        importZipApiId = apidtoZip.getId();
        HttpResponse createdZipApiResponse = restAPIPublisher.getAPI(importZipApiId);
        assertEquals(Response.Status.OK.getStatusCode(), createdZipApiResponse.getResponseCode());
    ...
    public void testGetWsdlDefinitions()throws Exception {
        ...
        ApiResponse<Void> response2 = restAPIPublisher.getWSDLSchemaDefinitionOfAPI(importZipApiId);
    ...
    public void testDownloadWsdlDefinitionsFromStore()throws Exception {
        ...
        //wsdl definition of the API created with .wsdl file from the store
        org.wso2.am.integration.clients.store.api.ApiResponse<Void> response2 = restAPIStore
