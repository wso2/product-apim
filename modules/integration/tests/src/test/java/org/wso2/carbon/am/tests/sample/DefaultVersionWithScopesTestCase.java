...
public class DefaultVersionWithScopesTestCase extends APIManagerIntegrationTest {
    ...
    public void testDefaultVersionAPIWithScopes() {
        ...
        try {
            ...
            response = HttpRequestUtil.doGet(getApiInvocationURLHttp(apiContext), requestHeaders);
            Assert.assertEquals(response.getResponseCode(), 200, "sam cannot access the GET Method. Response = "
                    + response.getData());
            ...
            response = HttpRequestUtil.doGet(getApiInvocationURLHttp(apiContext), requestHeaders);
            Assert.assertEquals(response.getResponseCode(), 403, "Mike should receive an HTTP 403 when trying to access"
