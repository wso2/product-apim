...
bcprov_text="bcprov-jdk15on,$BCPROV_JDK15ON_VERSION,../plugins/bcprov-jdk15on_$BCPROV_JDK15ON_VERSION.jar,4,true";
bcpkix_text="bcpkix-jdk15on,$BCPKIX_JDK15ON_VERSION,../plugins/bcpkix-jdk15on_$BCPKIX_JDK15ON_VERSION.jar,4,true";
homeDir="$HOME"
...
if [ "$ARGUMENT" = "DISABLE" ] || [ "$ARGUMENT" = "disable" ]; then
  ...
  if ! grep -q "$bcpkix_text" "$gateway_worker_bundles_info" ; then
	...
	if grep -q "$bcprov_text" "$api_publisher_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in api-publisher bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$api_publisher_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in api-publisher bundles.info. This should be removed";
  fi
	if grep -q "$bcprov_text" "$api_devportal_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in api-devportal bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$api_devportal_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in api-devportal bundles.info. This should be removed";
  fi
  if grep -q "$bcprov_text" "$api_key_manager_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in api-key-manager bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$api_key_manager_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in api-key-manager bundles.info. This should be removed";
  fi
  if grep -q "$bcprov_text" "$default_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in default bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$default_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in default bundles.info. This should be removed";
  fi
  if grep -q "$bcprov_text" "$control_plane_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in control-plane bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$control_plane_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in control-plane bundles.info. This should be removed";
  fi
  if grep -q "$bcprov_text" "$traffic_manager_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in traffic-manager bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$traffic_manager_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in traffic-manager bundles.info. This should be removed";
  fi
  if grep -q "$bcprov_text" "$gateway_worker_bundles_info" ; then
    verify=false
    echo  "Found $bcprov_text in gateway-worker bundles.info. This should be removed";
  fi
  if grep -q "$bcpkix_text" "$gateway_worker_bundles_info" ; then
    verify=false
    echo  "Found $bcpkix_text in gateway-worker bundles.info. This should be removed";
  fi
